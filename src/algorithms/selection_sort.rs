...
pub fn selection_sort(mut arr: Vec<u32>) -> Vec<u32> {
    ...
    for _ in 0..len {
        if len >= 1 {
            ...
            let smallest_idx = get_smallest_idx(&arr);
            // insert the smallest element into our new sorted_array variable
            sorted_array.push(arr[smallest_idx]);
...
pub fn get_smallest_idx(arr: &Vec<u32>) -> usize {
...
#[cfg(test)]
mod tests {
    use crate::algorithms::selection_sort::{get_smallest_idx, selection_sort};

    #[test]
    fn selection_sort_test() {
        let items = vec![9, 4, 6, 1, 5, 7, 3, 2, 8, 10];
        let empty: Vec<u32> = vec![];
        assert_eq!(3, get_smallest_idx(&items));
